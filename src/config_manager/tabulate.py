...
"""
Methods for calculating column widths
"""
WITH_ACCS = {
    "auto": min,
    "min": min,
...
def render_rst_table(column_names, column_align, column_sizing, rows):
    """
    ReStructuredText table
    """
    col_count = len(column_names)
    col_sep = " | "
    col_widths = calc_column_widths(column_names, rows, column_sizing)
...
def calc_column_widths(column_names, rows, column_sizing):
    header_widths = map(len, map(str, column_names))

    widths = [map(len, map(str, row)) for row in rows]

    accumulators = [WITH_ACCS[column_size] for column_size in column_sizing]
...
def horizontal_separator_line(col_widths, sep_char='-'):
    col_sep = "{}+{}".format(sep_char, sep_char)

    return (
        "+" + sep_char +
        render_horizontal_sep(col_sep, sep_char, col_widths) +
        sep_char + "+"
    )
