...
def setup_command_parser(subparsers):
    ...
    parser_extract = subparsers.add_parser(
        'init', help='initialize documentation file structure'
    )
...
def init_command(args):
    ...
    """
    copy_resource_tree(args.directory, 'config_manager', 'doc_template')
...
def copy_resource_tree(target_dir, package_name, resource_root,
    ...
    for resource in resource_listdir(
        package_name, os.path.join(resource_root, resource_dir)
    ):
        resource_path = os.path.join(resource_root, resource_dir, resource)

        if resource_isdir(package_name, resource_path):
            copy_resource_tree(
                target_dir, package_name, resource_root,
